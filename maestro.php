...
<body class="p-4 md:p-8 box-border flex flex-col h-screen overflow-hidden">
    ...
    <div id="modalRegistro" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-[rgba(0,0,0,0.6)] backdrop-blur-sm p-2 md:p-4">
        <div class="glass-panel w-full max-w-2xl p-5 md:p-8 relative max-h-[95vh] overflow-y-auto">
            ...
            <button onclick="document.getElementById('modalRegistro').classList.add('hidden')" class="absolute top-4 right-4 md:top-6 md:right-6 text-gray-400 hover:text-white transition-colors">
            ...
            <form class="flex flex-col gap-4" id="formNuevaIncidencia">
                <!-- Buscador Dinámico de Alumno -->
                <div class="flex flex-col gap-2 relative">
                    <label class="text-xs md:text-sm opacity-80">Buscador de Alumno</label>
                    <div class="relative">
                        <i class="ph ph-magnifying-glass absolute left-3 top-3.5 text-[var(--color-accent)] text-lg"></i>
                        <input type="text" id="buscadorAlumnosModal" placeholder="Escribe matrícula o nombre..." class="glass-input w-full pl-10 pr-4 py-3 text-sm" value="Juan P" autocomplete="off">
                        
                        <!-- Dropdown de coincidencias simulado -->
                        <div id="dropdownAlumnosModal" class="absolute w-full mt-1 glass-panel border border-[var(--color-accent)] rounded-lg overflow-hidden z-10 hidden" style="background-color: rgba(14, 20, 49, 0.95);">
                            <div class="p-3 border-b border-[var(--color-border)] hover:bg-[rgba(255,255,255,0.05)] cursor-pointer" onclick="selectAlumnoModal('Juan Pérez Martínez', 'MAT8374', '2do', 'B')">
                                <p class="text-sm font-medium">Juan Pérez Martínez <span class="text-xs opacity-60 ml-2">MAT8374</span></p>
                            </div>
                            <div class="p-3 hover:bg-[rgba(255,255,255,0.05)] cursor-pointer" onclick="selectAlumnoModal('Juan Pablo Silva', 'MAT5521', '3ro', 'A')">
                ...
                <div class="p-3 md:p-4 rounded-lg bg-[rgba(63,163,180,0.1)] border border-[var(--color-border)] flex items-center gap-3 md:gap-4">
                    <div class="h-10 w-10 md:h-12 md:w-12 rounded-full border border-[var(--color-border)] bg-[rgba(0,0,0,0.2)] flex items-center justify-center shrink-0">
                        <i class="ph ph-student text-[var(--color-accent)] text-xl md:text-2xl"></i>
                    </div>
                    <div class="min-w-0">
                        <p class="font-medium text-white text-sm md:text-base truncate" id="modalAlumnoNombre">Juan Pérez Martínez</p>
                        <p class="text-[11px] md:text-xs opacity-70" id="modalAlumnoDetalle">Grado: 2do | Grupo: "B" | Matrícula: MAT8374</p>
                    </div>
                </div>

                <!-- Selector de Categoría -->
                <div class="flex flex-col gap-1.5">
                    <label class="text-xs md:text-sm opacity-80">Categoría</label>
                    <div class="relative">
                        <select class="glass-input w-full pl-4 pr-10 py-2.5 md:py-3 text-sm appearance-none cursor-pointer" style="color-scheme: dark;">
                ...
                <div class="flex flex-col gap-1.5">
                    <label class="text-xs md:text-sm opacity-80">Descripción detallada</label>
                    <textarea class="glass-input w-full px-4 py-2.5 md:py-3 text-sm min-h-[90px] md:min-h-[120px] resize-y" placeholder="Describe claramente lo sucedido..."></textarea>
                </div>

                <!-- Botones Finales -->
                <div class="flex flex-col sm:flex-row gap-3 mt-2">
                    <button type="button" class="btn-flat flex-1 py-2.5 md:py-3 flex items-center justify-center gap-2 text-sm">
                        <i class="ph ph-image text-lg md:text-xl"></i> Adjuntar imagen
                    </button>
                    <button type="button" class="btn-accent flex-1 py-2.5 md:py-3 flex items-center justify-center gap-2 text-sm" onclick="document.getElementById('modalRegistro').classList.add('hidden')">
                        <i class="ph ph-floppy-disk text-lg md:text-xl"></i> Guardar Incidencia
                    </button>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        function selectAlumnoModal(nombre, matricula, grado, grupo) {
            document.getElementById('modalAlumnoNombre').textContent = nombre;
            document.getElementById('modalAlumnoDetalle').textContent = `Grado: ${grado} | Grupo: "${grupo}" | Matrícula: ${matricula}`;
            busqModal.value = nombre;
            dropModal.classList.add('hidden');
        }
