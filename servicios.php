...
<body class="p-4 md:p-6 box-border flex flex-col h-screen overflow-hidden">
    ...
    <div id="modalNueva" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-[rgba(0,0,0,0.65)] backdrop-blur-sm p-2 md:p-4">
        <div class="glass-panel w-full max-w-xl p-5 md:p-8 relative max-h-[95vh] overflow-y-auto tbl-scroll">
            <button onclick="cerrarModalNueva()" class="absolute top-5 right-5 text-gray-400 hover:text-white transition-colors">
                <i class="ph ph-x text-xl"></i>
            </button>
            <h3 class="text-lg font-semibold mb-1 text-[var(--color-accent)] flex items-center gap-2">
            ...
            <form id="formNuevaIncidencia" class="flex flex-col gap-4" novalidate>
                ...
                <fieldset class="flex flex-col gap-3 p-3 md:p-4 rounded-lg border border-[var(--color-border)] bg-[rgba(255,255,255,0.02)]">
                    ...
                    <div class="flex flex-col gap-1.5 relative">
                        ...
                        <div id="dropdownAlumnosModal" class="absolute top-full left-0 w-full mt-1 glass-panel border border-[var(--color-accent)] rounded-lg overflow-hidden z-20 hidden" style="background-color: rgba(10,15,40,0.97);"></div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <input type="number" id="fi_grado" name="grado" min="1" max="6" class="glass-input w-full px-4 py-2.5 text-sm" placeholder="Ej. 2" required>
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="text-xs opacity-70">Grupo <span class="text-red-400">*</span></label>
                            <input type="text" id="fi_grupo" name="grupo" class="glass-input w-full px-4 py-2.5 text-sm" placeholder="Ej. A" required>
                        </div>
                ...
                <div class="flex flex-col sm:flex-row gap-3 mt-1">
                    <button type="button" id="btnCancelarIncidencia" class="btn-flat flex-1 py-2.5 md:py-3 flex items-center justify-center gap-2 text-sm">
    ...
    <div id="modalDetalle" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-[rgba(0,0,0,0.65)] backdrop-blur-sm p-2 md:p-4">
        ...
        <script>
        // ── Variables del modal nueva incidencia ────────────────────────
        ...
        function cerrarModalNueva() { document.getElementById('modalNueva').classList.add('hidden'); resetFormIncidencia(); }
        document.getElementById('btnCancelarIncidencia').addEventListener('click', cerrarModalNueva);
        document.addEventListener('click', (e) => { if (!busqModal.contains(e.target) && !dropModal.contains(e.target)) { dropModal.classList.add('hidden'); } });
        function resetFormIncidencia() { document.getElementById('formNuevaIncidencia').reset(); document.getElementById('fi_id_alumno').value=''; document.getElementById('modalAlumnoNombre').textContent='Sin alumno seleccionado'; document.getElementById('modalAlumnoDetalle').textContent='Busca o llena los campos manualmente abajo'; dropModal.classList.add('hidden'); ocultarMsg(); }
        function mostrarMsg(texto, tipo) { const el = document.getElementById('msgIncidencia'); el.textContent = texto; el.className = `mb-4 px-4 py-3 rounded-lg text-sm font-medium ${tipo === 'success' ? 'bg-[rgba(52,211,153,0.15)] border border-[rgba(52,211,153,0.4)] text-green-300' : 'bg-[rgba(248,113,113,0.15)] border border-[rgba(248,113,113,0.4)] text-red-300'}`; el.classList.remove('hidden'); }
        function ocultarMsg() { document.getElementById('msgIncidencia').classList.add('hidden'); }
        function rellenarFichaAlumno(a) { document.getElementById('fi_id_alumno').value = a.id_alumno; document.getElementById('fi_matricula').value = a.matricula; document.getElementById('fi_nombre').value = a.nombre; document.getElementById('fi_apellidos').value = a.apellidos; document.getElementById('fi_grado').value = a.grado; document.getElementById('fi_grupo').value = a.grupo; document.getElementById('fi_correo').value = a.correo_alumno; document.getElementById('fi_tutor').value = a.datos_tutor; document.getElementById('modalAlumnoNombre').textContent = `${a.nombre} ${a.apellidos}`; document.getElementById('modalAlumnoDetalle').textContent = `Grado: ${a.grado} | Grupo: ${a.grupo} | Matrícula: ${a.matricula}`; busqModal.value = `${a.nombre} ${a.apellidos}`; dropModal.classList.add('hidden'); }
                ...
                .then(data=>{ 
                    ...
                    if (!Array.isArray(data) || data.length===0) { 
                        ...
                        data.forEach(a=>{ 
                            ...
                            item.innerHTML = `<p class="text-sm font-medium">${a.nombre} ${a.apellidos} <span class="text-xs opacity-50 ml-2">${a.matricula}</span></p><p class="text-[11px] opacity-50">${a.grado}° grado, Grupo ${a.grupo}</p>`; 
        ...
        document.getElementById('formNuevaIncidencia').addEventListener('submit', function(e){ 
            ...
            .then(res=>{ 
                if(res.status==='success'){ 
                    ...
                    setTimeout(()=>cerrarModalNueva(),2200); 
                } else { 
